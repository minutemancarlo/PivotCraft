...
import { Download, RefreshCw, ExternalLink, X, CheckCircle2, AlertTriangle, Sparkles, Check } from 'lucide-react';
...
export const UpdateModal: React.FC<UpdateModalProps> = ({
  ...
  const isUpToDate = status === 'not-available' || (updateInfo && updateInfo.version === currentVersion);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-150">
      ...
      >
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
          {isUpToDate ? (
            <div
              className={`p-5 rounded-xl border flex items-center space-x-3.5 ${
            ...
            >
              ...
              <div>
                <h4 className="text-sm font-bold text-emerald-400">You're up to date!</h4>
                <p className="text-xs opacity-75 mt-0.5">
                  PivotCraft v{currentVersion} is currently the latest version.
                </p>
              </div>
            </div>
          ) : (
            <>
              ...
              {/* Downloaded State */}
              {status === 'downloaded' && (
                <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center space-x-2.5 text-xs font-medium">
                  <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0" />
                  <span>Update downloaded. Restart PivotCraft now to apply the new version.</span>
                </div>
              )}
        ...
        >
          ...
          >
            {isUpToDate ? 'Close' : status === 'downloaded' ? 'Update on Next Quit' : 'Dismiss'}
          </button>
          ...
          {!isUpToDate && (
            <div className="flex items-center space-x-2">
              {status === 'downloaded' ? (
                <button
                  onClick={onInstall}
                  className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-md shadow-emerald-950/40 flex items-center space-x-1.5 cursor-pointer transition"
                >
